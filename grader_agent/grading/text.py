...
def escala_item_desde_rubrica(rubrica_md: str, pregunta: str) -> dict:
    """
    Ubica el ítem en la rúbrica y devuelve puntaje máximo canónico y niveles opcionales.
    """
...
"""
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": system_prompt_texto_escala_item()},
            {"role": "user", "content": user_message},
        ],
        temperature=0,
        response_format={"type": "json_object"},
        **chat_completion_limit_kwargs(kind="escala"),
    )
...
"""
    ...
    r_p = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": system_prompt_texto_puntaje_item()},
    ...
    data_p = json_object_from_message_content(r_p.choices[0].message.content)
    try:
...
"""
    ...
    r_r = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": system_prompt_texto_retro_item()},
    ...
    data_r = json_object_from_message_content(r_r.choices[0].message.content)
